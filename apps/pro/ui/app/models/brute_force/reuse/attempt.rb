# A proposed or accomplished attempt to use {#metasploit_credential_core} to login into {#service} as part of a
...
class BruteForce::Reuse::Attempt < ApplicationRecord
  #
  # Associations
  #
  ...
  belongs_to :brute_force_run,
             ...
             inverse_of: :brute_force_reuse_attempts

  # @!attribute metasploit_credential_core
  #   The core credential being used to log into {#service}.
  #
  #   @return [Metasploit::Credential::Core]
  belongs_to :metasploit_credential_core,
  ...
  validates :brute_force_run,
            presence: true
  validates :metasploit_credential_core,
            presence: true
  validates :service,
            presence: true
  ...
  # {Metasploit::Credential::Core#to_credential} on the associated
  # {Metasploit::Credential::Core}.
  #
  # @return {Metasploit::Framework::Credential}
  def to_credential
    credential = self.metasploit_credential_core.to_credential
  ...
  # Validates that `Metasploit::Credential::Core#workspace` of {#metasploit_credential_core} matches
  # `Mdm::Host#workspace` of `Mdm::Service#host` of {#service}.
  def consistent_workspaces
    unless metasploit_credential_core.try(:workspace) == service.try(:host).try(:workspace)
