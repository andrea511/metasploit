define [
  'base_controller'
  'apps/services/delete/delete_view'
  'lib/concerns/controllers/table_selections'
  'lib/components/flash/flash_controller'
], ->
  @Pro.module "ServicesApp.Delete", (Delete, App, Backbone, Marionette, $, _) ->
    class Delete.Controller extends App.Controllers.Application
      ...
      onFormSubmit: ->
        ...
        jQuery.ajax
          url: Routes.destroy_multiple_workspace_services_path(workspace_id: WORKSPACE_ID)
          type: 'DELETE'
          data:
            selections:
          ...
          success: =>
            ...
            App.execute 'flash:display',
              title:    "Service#{ @pluralizedMessage '', 's' } deleted"
              message:  "The service#{ @pluralizedMessage ' was', 's were' } successfully deleted."

          error: =>
            App.execute 'flash:display',
              title:   'An error occurred'
              style:   'error'
              message: "There was a problem deleting the selected service#{ if @multipleSelected() then 's' }"
