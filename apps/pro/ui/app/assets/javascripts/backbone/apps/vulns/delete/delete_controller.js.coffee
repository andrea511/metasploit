# TODO: This is almost identical to the creds version. This should form a nice basis for IA abstraction work later on.
define [
  'base_controller'
  'apps/vulns/delete/delete_view'
  'lib/concerns/controllers/table_selections'
  'lib/components/flash/flash_controller'
], ->
  @Pro.module "VulnsApp.Delete", (Delete, App, Backbone, Marionette, $, _) ->
    class Delete.Controller extends App.Controllers.Application
      ...
      onFormSubmit: ->
        ...
        jQuery.ajax
          url: Routes.destroy_multiple_workspace_vulns_path(workspace_id: WORKSPACE_ID)
          type: 'DELETE'
          data:
            selections:
          ...
          success: =>
            ...
            App.execute 'flash:display',
              title:    "Vulnerabilit#{ @pluralizedMessage 'y', 'ies' } deleted"
              message:  "The vulnerabilit#{ @pluralizedMessage 'y was', 'ies were ' } successfully deleted."

          error: =>
            App.execute 'flash:display',
              title:   'An error occurred'
              style:   'error'
              message: "There was a problem deleting the selected vulnerabilit#{ if @multipleSelected() then 'ies' else 'y' }"
