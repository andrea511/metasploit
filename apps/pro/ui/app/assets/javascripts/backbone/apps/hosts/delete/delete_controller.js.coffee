define [
  'base_controller'
  'apps/hosts/delete/delete_view'
  'lib/concerns/controllers/table_selections'
  'lib/components/flash/flash_controller'
], ->
  @Pro.module "HostsApp.Delete", (Delete, App, Backbone, Marionette, $, _) ->
    class Delete.Controller extends App.Controllers.Application
      ...
      onFormSubmit: ->
        ...
        jQuery.ajax
          url: Routes.destroy_multiple_hosts_path(workspace_id: WORKSPACE_ID)
          type: 'DELETE'
          data:
            selections:
          ...
          success: =>
            ...
            App.execute 'flash:display',
              title:    "Host#{ @pluralizedMessage '', 's' } deleted"
              message:  "The host#{ @pluralizedMessage ' was', 's were ' } successfully deleted."

          error: =>
            App.execute 'flash:display',
              title:   'An error occurred'
              style:   'error'
              message: "There was a problem deleting the selected host#{ if @multipleSelected() then 's' }"
