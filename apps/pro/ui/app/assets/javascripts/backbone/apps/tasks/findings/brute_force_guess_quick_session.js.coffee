define [
  'css!css/brute_force_guess_findings'
  'lib/components/table/cell_views'
  'lib/shared/attempt_session/attempt_session_controller'
  'apps/creds/findings/findings_controller'
  ...
  @Pro.module 'TasksApp.Findings', (Findings, App) ->
    ...
    hostView = Backbone.Marionette.ItemView.extend
      template: (m) ->
        "<a href='/hosts/#{_.escape m.host_id}' class='underline'>#{_.escape m.address}</a>"
    ...
    goToSession = Backbone.Marionette.ItemView.extend
      template: (m) ->
        if m.session_id
         "<a href='#{Routes.session_path(WORKSPACE_ID,m.session_id)}' class='underline'>Session #{m.session_id}</a>"
        else
          ''
    ...
    Findings.BruteForceGuessQuickSession =
      ...
      tables:
        login_attempts:
          ...
          columns: [
            ...
            { label: 'Public',           attribute: 'public', view: publicView }
            { label: 'Private',          attribute: 'private', view: privateView }
            { label: 'Realm',            attribute: 'realm', view: realmView, sortAttribute: 'realm_key'}
        ...
        targets_compromised:
          columns: [
            ...
            { label: "OS", attribute: "host_os_name"}
              ...
              hoverView:  Pro.Creds.CellViews.CollectionHover.extend(
                url: ->
                  Routes.task_detail_path(
                    WORKSPACE_ID,
                    TASK_ID
                  ) + "/stats/successful_logins_hover.json?service_id=#{@model.get('attempt_ids')}"
                title: ->
                  ""
                columns: [
                  { label: 'Public', size: 4, attribute: 'public_username' }
                  { label: 'Private', size: 5, attribute: 'private_data' }
