...
jQuery ($) ->
  window.TaskChain = class TaskChain
    ...
    # returns true if task number is a Report
    @taskIsReport: (taskNumber)->
     $("#" + TaskChain.configId(taskNumber)).find('#report_option_forms').length > 0
    

    # Return only the form for the chosen kind of report
    @chosenReportConfigForTask: (taskConfig)->
      taskType = taskConfig.find('select#report_selector').val()
      taskType = taskType.toLowerCase()
      taskConfig.find("#report_option_forms #report_form_#{taskType}")
...
# ------------------------ ELEMENT BEHAVIOR -----------------------------------
    ...
    @elementBehaviors:
      ...
      # Remove the report generation buttons on report task selection.
      reportTaskPreparer:
        bind: ->
          $('body').bind 'reportTaskLoaded', ->
            $('#report_option_forms').find('fieldset.buttons').remove()
        ...
        bind: ->
          $(TaskChain.mainFormSelector).submit ->
            ...
            if $(TaskChain.container).find('li').length is 0
              ...
              # Get all task configs except report, which has to be treated specially
              $(".task-config-wrapper").each (index, configDiv) ->
                $configDiv = $(configDiv)
                return null if $configDiv.attr('tasktype') == "report" # we'll get reports stuff another way
              ...
              # Get the report task config
              if TaskChain.hasTaskType('report')
                $('.task-config-wrapper[tasktype="report"]').each (index, configDiv) ->
                  $configDiv = $(configDiv)
                  dataSnowball = dataSnowball.concat TaskChain.chosenReportConfigForTask($configDiv).find(dataElementSelector)
