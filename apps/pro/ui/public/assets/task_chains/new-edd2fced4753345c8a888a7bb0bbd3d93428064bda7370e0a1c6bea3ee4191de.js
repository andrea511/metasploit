    ...
    window.TaskChain = TaskChain = (function() {
      ...
      TaskChain.loadTaskConfigForm = function(taskType, taskNumber) {
        ...
        filePath = (function() {
          switch (taskType) {
            case "module_run":
              return "/workspaces/" + wId + "/modules";
            case "report":
              return "/workspaces/" + wId + "/reports/new";
            default:
              return "/workspaces/" + wId + "/tasks/new_" + taskType;
          }
        })();
        args = {
          _nl: "1"
      ...
      TaskChain.reportNameForChain = function(element, taskNum) {
        return element.name = element.name.replace(/report\[/, "inputs_for_task[" + taskNum + "][");
      };

      TaskChain.transformReportViewForChain = function() {
        var container, input, inputs, reportSubmitButton, select, selects, taskNum, _i, _j, _len, _len1, _results;
        ...
        inputs = $.grep(container.find("input"), function(element, index) {
          return $.inArray(element.name, TaskChain.inputSkipNames) < 0;
        });
        for (_i = 0, _len = inputs.length; _i < _len; _i++) {
          input = inputs[_i];
          TaskChain.reportNameForChain(input, taskNum);
        }
        selects = $.grep(container.find("select"), function(element, index) {
          return $.inArray(element.name, TaskChain.selectSkipNames) < 0;
        });
        _results = [];
        for (_j = 0, _len1 = selects.length; _j < _len1; _j++) {
          select = selects[_j];
          _results.push(TaskChain.reportNameForChain(select, taskNum));
        }
        return _results;
      };
      ...
      TaskChain.elementBehaviors = {
        ...
        reportSubmitHider: {
          bind: function() {
            return $('body').bind('reportLoaded', function() {
              return TaskChain.transformReportViewForChain();
            });
          }
        },
