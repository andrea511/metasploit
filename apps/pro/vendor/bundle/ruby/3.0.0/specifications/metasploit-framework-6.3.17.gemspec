# -*- encoding: utf-8 -*-
# stub: metasploit-framework 6.3.17 ruby lib

Gem::Specification.new do |s|
  s.name = "metasploit-framework".freeze
  s.version = "6.3.17"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["Metasploit Hackers".freeze]
  s.bindir = ".".freeze
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n".freeze, "-----BEGIN CERTIFICATE-----\\nB4H6i9r5gkn3Ym6hU/oSlBiFLpKR6mhsRDKyZqHnGKSaZFHv\n-----END CERTIFICATE-----\n".freeze, "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n".freeze]
  s.date = "2023-05-18"
  s.description = "metasploit-framework".freeze
  s.email = ["msfdev@metasploit.com".freeze]
  s.executables = ["msfconsole".freeze, "msfd".freeze, "msfrpc".freeze, "msfrpcd".freeze, "msfvenom".freeze]
  s.files = ["./msfconsole".freeze, "./msfd".freeze, "./msfrpc".freeze, "./msfrpcd".freeze, "./msfvenom".freeze]
  s.homepage = "https://www.metasploit.com".freeze
  s.licenses = ["BSD-3-clause".freeze]
  s.required_ruby_version = Gem::Requirement.new(">= 2.7".freeze)
  s.rubygems_version = "3.2.22".freeze
  s.summary = "metasploit-framework".freeze

  s.installed_by_version = "3.2.22" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4
  end

  if s.respond_to? :add_runtime_dependency then
    s.add_runtime_dependency(%q<activerecord>.freeze, ["~> 7.0"])
    s.add_runtime_dependency(%q<activesupport>.freeze, ["~> 7.0"])
    s.add_runtime_dependency(%q<actionpack>.freeze, ["~> 7.0"])
    s.add_runtime_dependency(%q<bcrypt>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<bootsnap>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<jsobfu>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<json>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<metasm>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<metasploit-concern>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<metasploit-credential>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<metasploit_data_models>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<metasploit-model>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<metasploit-payloads>.freeze, ["= 2.0.130"])
    s.add_runtime_dependency(%q<metasploit_payloads-mettle>.freeze, ["= 1.0.20"])
    s.add_runtime_dependency(%q<msgpack>.freeze, ["~> 1.6.0"])
    s.add_runtime_dependency(%q<network_interface>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rubyntlm>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<nokogiri>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<packetfu>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<pcaprub>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<pg>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<railties>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<recog>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<openssl-ccm>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<octokit>.freeze, ["~> 4.0"])
    s.add_runtime_dependency(%q<redcarpet>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<patch_finder>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<puma>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<thin>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<sinatra>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<warden>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<swagger-blocks>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<em-http-request>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<tzinfo-data>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<sshkey>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<windows_error>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<xmlrpc>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<http-cookie>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<unix-crypt>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rasn1>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<pdf-reader>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<ruby-macho>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<bson>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<dnsruby>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<mqtt>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<net-ssh>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<ed25519>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<bcrypt_pbkdf>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<ruby_smb>.freeze, ["~> 3.2.0"])
    s.add_runtime_dependency(%q<net-ldap>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<net-smtp>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<winrm>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-core>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-text>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-random_identifier>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-powershell>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-zip>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-registry>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-java>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-struct2>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-arch>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-ole>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-mime>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-nop>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-bin_tools>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-socket>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-sslscan>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-rop_builder>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-encoder>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rex-exploitation>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rb-readline>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<rubyzip>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<sqlite3>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<tzinfo>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<filesize>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<openvas-omp>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<nessus_rest>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<nexpose>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<xdr>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<faker>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<hrr_rb_ssh-ed25519>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<irb>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<aws-sdk-s3>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<aws-sdk-ec2>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<aws-sdk-iam>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<faye-websocket>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<eventmachine>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<faraday>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<faraday-retry>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<win32api>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<zeitwerk>.freeze, [">= 0"])
    s.add_runtime_dependency(%q<chunky_png>.freeze, [">= 0"])
  else
    s.add_dependency(%q<activerecord>.freeze, ["~> 7.0"])
    s.add_dependency(%q<activesupport>.freeze, ["~> 7.0"])
    s.add_dependency(%q<actionpack>.freeze, ["~> 7.0"])
    s.add_dependency(%q<bcrypt>.freeze, [">= 0"])
    s.add_dependency(%q<bootsnap>.freeze, [">= 0"])
    s.add_dependency(%q<jsobfu>.freeze, [">= 0"])
    s.add_dependency(%q<json>.freeze, [">= 0"])
    s.add_dependency(%q<metasm>.freeze, [">= 0"])
    s.add_dependency(%q<metasploit-concern>.freeze, [">= 0"])
    s.add_dependency(%q<metasploit-credential>.freeze, [">= 0"])
    s.add_dependency(%q<metasploit_data_models>.freeze, [">= 0"])
    s.add_dependency(%q<metasploit-model>.freeze, [">= 0"])
    s.add_dependency(%q<metasploit-payloads>.freeze, ["= 2.0.130"])
    s.add_dependency(%q<metasploit_payloads-mettle>.freeze, ["= 1.0.20"])
    s.add_dependency(%q<msgpack>.freeze, ["~> 1.6.0"])
    s.add_dependency(%q<network_interface>.freeze, [">= 0"])
    s.add_dependency(%q<rubyntlm>.freeze, [">= 0"])
    s.add_dependency(%q<nokogiri>.freeze, [">= 0"])
    s.add_dependency(%q<packetfu>.freeze, [">= 0"])
    s.add_dependency(%q<pcaprub>.freeze, [">= 0"])
    s.add_dependency(%q<pg>.freeze, [">= 0"])
    s.add_dependency(%q<railties>.freeze, [">= 0"])
    s.add_dependency(%q<recog>.freeze, [">= 0"])
    s.add_dependency(%q<openssl-ccm>.freeze, [">= 0"])
    s.add_dependency(%q<octokit>.freeze, ["~> 4.0"])
    s.add_dependency(%q<redcarpet>.freeze, [">= 0"])
    s.add_dependency(%q<patch_finder>.freeze, [">= 0"])
    s.add_dependency(%q<puma>.freeze, [">= 0"])
    s.add_dependency(%q<thin>.freeze, [">= 0"])
    s.add_dependency(%q<sinatra>.freeze, [">= 0"])
    s.add_dependency(%q<warden>.freeze, [">= 0"])
    s.add_dependency(%q<swagger-blocks>.freeze, [">= 0"])
    s.add_dependency(%q<em-http-request>.freeze, [">= 0"])
    s.add_dependency(%q<tzinfo-data>.freeze, [">= 0"])
    s.add_dependency(%q<sshkey>.freeze, [">= 0"])
    s.add_dependency(%q<windows_error>.freeze, [">= 0"])
    s.add_dependency(%q<xmlrpc>.freeze, [">= 0"])
    s.add_dependency(%q<http-cookie>.freeze, [">= 0"])
    s.add_dependency(%q<unix-crypt>.freeze, [">= 0"])
    s.add_dependency(%q<rasn1>.freeze, [">= 0"])
    s.add_dependency(%q<pdf-reader>.freeze, [">= 0"])
    s.add_dependency(%q<ruby-macho>.freeze, [">= 0"])
    s.add_dependency(%q<bson>.freeze, [">= 0"])
    s.add_dependency(%q<dnsruby>.freeze, [">= 0"])
    s.add_dependency(%q<mqtt>.freeze, [">= 0"])
    s.add_dependency(%q<net-ssh>.freeze, [">= 0"])
    s.add_dependency(%q<ed25519>.freeze, [">= 0"])
    s.add_dependency(%q<bcrypt_pbkdf>.freeze, [">= 0"])
    s.add_dependency(%q<ruby_smb>.freeze, ["~> 3.2.0"])
    s.add_dependency(%q<net-ldap>.freeze, [">= 0"])
    s.add_dependency(%q<net-smtp>.freeze, [">= 0"])
    s.add_dependency(%q<winrm>.freeze, [">= 0"])
    s.add_dependency(%q<rex-core>.freeze, [">= 0"])
    s.add_dependency(%q<rex-text>.freeze, [">= 0"])
    s.add_dependency(%q<rex-random_identifier>.freeze, [">= 0"])
    s.add_dependency(%q<rex-powershell>.freeze, [">= 0"])
    s.add_dependency(%q<rex-zip>.freeze, [">= 0"])
    s.add_dependency(%q<rex-registry>.freeze, [">= 0"])
    s.add_dependency(%q<rex-java>.freeze, [">= 0"])
    s.add_dependency(%q<rex-struct2>.freeze, [">= 0"])
    s.add_dependency(%q<rex-arch>.freeze, [">= 0"])
    s.add_dependency(%q<rex-ole>.freeze, [">= 0"])
    s.add_dependency(%q<rex-mime>.freeze, [">= 0"])
    s.add_dependency(%q<rex-nop>.freeze, [">= 0"])
    s.add_dependency(%q<rex-bin_tools>.freeze, [">= 0"])
    s.add_dependency(%q<rex-socket>.freeze, [">= 0"])
    s.add_dependency(%q<rex-sslscan>.freeze, [">= 0"])
    s.add_dependency(%q<rex-rop_builder>.freeze, [">= 0"])
    s.add_dependency(%q<rex-encoder>.freeze, [">= 0"])
    s.add_dependency(%q<rex-exploitation>.freeze, [">= 0"])
    s.add_dependency(%q<rb-readline>.freeze, [">= 0"])
    s.add_dependency(%q<rubyzip>.freeze, [">= 0"])
    s.add_dependency(%q<sqlite3>.freeze, [">= 0"])
    s.add_dependency(%q<tzinfo>.freeze, [">= 0"])
    s.add_dependency(%q<filesize>.freeze, [">= 0"])
    s.add_dependency(%q<openvas-omp>.freeze, [">= 0"])
    s.add_dependency(%q<nessus_rest>.freeze, [">= 0"])
    s.add_dependency(%q<nexpose>.freeze, [">= 0"])
    s.add_dependency(%q<xdr>.freeze, [">= 0"])
    s.add_dependency(%q<faker>.freeze, [">= 0"])
    s.add_dependency(%q<hrr_rb_ssh-ed25519>.freeze, [">= 0"])
    s.add_dependency(%q<irb>.freeze, [">= 0"])
    s.add_dependency(%q<aws-sdk-s3>.freeze, [">= 0"])
    s.add_dependency(%q<aws-sdk-ec2>.freeze, [">= 0"])
    s.add_dependency(%q<aws-sdk-iam>.freeze, [">= 0"])
    s.add_dependency(%q<faye-websocket>.freeze, [">= 0"])
    s.add_dependency(%q<eventmachine>.freeze, [">= 0"])
    s.add_dependency(%q<faraday>.freeze, [">= 0"])
    s.add_dependency(%q<faraday-retry>.freeze, [">= 0"])
    s.add_dependency(%q<win32api>.freeze, [">= 0"])
    s.add_dependency(%q<zeitwerk>.freeze, [">= 0"])
    s.add_dependency(%q<chunky_png>.freeze, [">= 0"])
  end
end
